...
import { defaultReaderContent, getCurrentReaderSentence, getReaderContent, getReaderStats, readerContents, type ReaderContent } from '../features/reader/catalog'
...
export function Reader({ allWords, progress, interactions, onInteraction, onSaveWord, onGoReview }: {
  ...
  const readingView = <>
    <ReaderToolbar content={currentContent} progress={document.scrollProgress} currentSentence={currentSentence} settingsOpen={settingsOpen} onToggleSettings={() => setSettingsOpen(value => !value)} onEdit={openEditor} onLibrary={() => { setSettingsOpen(false); setSelectedToken(''); setView('library') }} settingsPanel={settingsPanel}/>
    ...
    {document.text.trim() && document.scrollProgress >= 100 && <ReaderCompletion content={currentContent} progress={document.scrollProgress} encounteredCount={readerStats.encounteredCount} savedCount={readerStats.savedCount} onReview={onGoReview} onContinue={openNextContent} hasNext={hasNextContent}/>}
  </>
