...
import { examRoutes, type ExamRouteId } from '../../features/exams/routes'
import { ReaderContentCard } from './ReaderContentCard'

export function ReaderLibrary({ contents, currentContent, document, onContinue, onSelectContent, onOpenEditor, onOpenImport, importTriggerRef, onUseExample }: {
  ...
  const [selectedExamRoute, setSelectedExamRoute] = useState<ExamRouteId | 'all'>('all')
  const filteredContents = filterReaderContents(contents, filters)
  const displayedContents = selectedExamRoute === 'all' ? filteredContents : filteredContents.filter(content => content.examId === selectedExamRoute)
  const updateFilter = <K extends keyof ReaderFilters>(key: K, value: ReaderFilters[K]) => {
    setFilters(previous => ({ ...previous, [key]: value }))
    if (key === 'track') setSelectedExamRoute('all')
  }
  ...
  return <div className="readerLibrary pageEnter">
    ...
    <section className="readerRecommendations" aria-labelledby="reader-recommendations-title"><div className="sectionHeading"><div><span className="eyebrow">RECOMMENDED</span><h2 id="reader-recommendations-title">推荐给你</h2></div><span>{displayedContents.length} 篇本地短文</span></div><div className="readerContentGrid">{displayedContents.map(content => <ReaderContentCard key={content.id} content={content} active={content.id === currentContent.id} onOpen={() => content.id === currentContent.id ? onContinue() : onSelectContent(content)}/>)}</div>{!displayedContents.length && <div className="readerFilterEmpty"><Filter size={20}/><p>没有符合当前筛选的内容。</p><button className="textButton" type="button" onClick={() => { setFilters(defaultReaderFilters); setSelectedExamRoute('all') }}>清除筛选</button></div>}</section>
    ...
    <section className="readerExamRoutes" aria-labelledby="reader-exam-title"><div className="sectionHeading"><div><span className="eyebrow">EXAM STARTERS</span><h2 id="reader-exam-title">选择考试路线</h2></div><span>本地 A2 / B1 起始内容</span></div><div className="readerExamRouteGrid">{examRoutes.map(route => <button key={route.id} className={`readerExamRoute ${selectedExamRoute === route.id ? 'active' : ''}`} type="button" onClick={() => { setSelectedExamRoute(route.id); setFilters({ ...defaultReaderFilters, track: 'exam' }) }}><span><strong>{route.label}</strong><small>{route.starterLevel} 起点</small></span><p>{route.description}</p></button>)}</div></section>
    <section className="readerCustomEntry" aria-labelledby="reader-custom-title"><div><span className="eyebrow">YOUR TEXT</span><h2 id="reader-custom-title">找不到想读的内容？</h2><p>粘贴一篇自己的英文，Lexora 会继续保存同一份阅读进度和查词记录。</p></div><div className="readerCustomActions"><button className="secondaryButton" type="button" onClick={onOpenEditor}><Pencil size={16}/> 粘贴自己的英文</button><button ref={importTriggerRef} className="secondaryButton" type="button" onClick={onOpenImport}>导入 URL / 字幕</button><button className="textButton" type="button" onClick={onUseExample}>使用示例文章</button></div></section>
  </div>
}
