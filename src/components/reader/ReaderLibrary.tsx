...
export function ReaderLibrary({ contents, currentContent, document, onContinue, onSelectContent, onOpenEditor, onUseExample }: {
  ...
  return <div className="readerLibrary pageEnter">
    ...
    <section className="readerRecommendations" aria-labelledby="reader-recommendations-title"><div className="sectionHeading"><div><span className="eyebrow">RECOMMENDED</span><h2 id="reader-recommendations-title">推荐给你</h2></div><span>{filteredContents.length} 篇本地短文</span></div><div className="readerContentGrid">{filteredContents.map(content => <ReaderContentCard key={content.id} content={content} active={content.id === currentContent.id} onOpen={() => onSelectContent(content)}/>)}</div>{!filteredContents.length && <div className="readerFilterEmpty"><Filter size={20}/><p>没有符合当前筛选的内容。</p><button className="textButton" type="button" onClick={() => setFilters(defaultReaderFilters)}>清除筛选</button></div>}</section>
    <section className="readerFilterSection" aria-labelledby="reader-filter-title"><div className="sectionHeading"><div><span className="eyebrow">FILTER</span><h2 id="reader-filter-title">轻量筛选</h2></div><span>只筛选当前本地内容</span></div><div className="readerFilterBar"><label>难度<select value={filters.level} onChange={event => updateFilter('level', event.target.value as ReaderFilters['level'])}><option value="all">全部难度</option><option value="A1">A1</option><option value="A2">A2</option><option value="B1">B1</option></select></label><label>方向<select value={filters.track} onChange={event => updateFilter('track', event.target.value as ReaderFilters['track'])}><option value="all">全部方向</option><option value="daily">日常英语</option><option value="exam">考试英语</option><option value="shared">共享</option></select></label><label>主题<select value={filters.topic} onChange={event => updateFilter('topic', event.target.value as ReaderFilters['topic'])}><option value="all">全部主题</option><option value="生活">生活</option><option value="工作">工作</option><option value="旅行">旅行</option><option value="学习">学习</option><option value="观点">观点</option></select></label><label>类型<select value={filters.kind} onChange={event => updateFilter('kind', event.target.value as ReaderFilters['kind'])}><option value="all">全部类型</option><option value="article">短文</option><option value="dialogue">对话</option><option value="email">邮件</option><option value="story">故事</option></select></label></div></section>
