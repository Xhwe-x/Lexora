      ...
      <div className="placementFrame placementAssessmentFrame">
        ...
        <section className="placementCard placementQuestionCard" aria-labelledby="placement-question-title">
          ...
          <form onSubmit={event => { event.preventDefault(); submitAnswer() }}>
            ...
            {submittedAnswer && currentQuestion.skill === 'context' && <div className={`placementAnswerFeedback ${submittedAnswer.correct ? 'correct' : 'incorrect'}`} role="status" aria-live="polite"><strong>{submittedAnswer.correct ? '回答正确' : '这次差一点'}</strong><p>{submittedAnswer.correct ? '完整句子：' : '正确句子：'}{currentQuestion.feedback ?? currentQuestion.prompt}</p><span>中文解释：{currentQuestion.explanation ?? currentQuestion.contextHint}</span>{!submittedAnswer.correct && <small>错误原因：先根据句子表达的意思判断动词，再确认词形。</small>}</div>}
